...
public class ventanaEliminar {
    ...
    JButton log;
    public static int usu_eliminado=0;
...
//System.out.println(x+" "+y);
        JLabel title = new JLabel("                  Eliminar Usuario"); ////YOUR TITLE
        JLabel title1 = new JLabel("                  Introduzca el usuario que desea eliminar");
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                    ...
                    if (entrar_menu) {
                        for (int i = 0; i < Main.usuArray.size(); i++) {
                            if (Main.usuArray.get(i).getUsername().equals(user) && Main.usuArray.get(i).getPassword().equals(passw)) {
                                JOptionPane.showMessageDialog(null, "Está seguro que desea eliminar el usuario: "+user+"?");
                                //if () {
